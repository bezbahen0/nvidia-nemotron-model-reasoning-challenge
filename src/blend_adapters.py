...
def main():
    ...
    wandb.init(
        job_type="blending",
        name=f"{args.wandb_run_basename}-model-soup-creation",
    ...
    logger.info(f"🏆 Лучший чекпоинт: {best_ckpt_name} с Accuracy = {best_score:.4f}")
    logger.info(f"Копирование лучшего адаптера в {args.output_best}...")
    
    if Path(args.output_best).exists():
        shutil.rmtree(args.output_best)
    shutil.copytree(best_ckpt_path, args.output_best)
    logger.info("✅ Лучший чекпоинт сохранен.\n")
    ...
    if len(top_k_checkpoints) < 2:
        logger.info("Недостаточно чекпоинтов с Accuracy > 0 для создания Model Soup.")
        wandb.finish()
        return

    logger.info(f"🍲 Создание Model Soup из Топ-{len(top_k_checkpoints)} чекпоинтов:")
    ...
    for name, score in top_k_checkpoints:
        ...
        logger.info(f" -> {name}: score = {score:.4f} | вес = {weight:.4f}")
    ...
    logger.info("Загрузка основного адаптера...")
    ...
    for i, adapter_path in enumerate(adapters_to_merge[1:], start=1):
        ...
        logger.info(f"Загрузка {adapter_name}...")
    ...
    logger.info(f"Сохранение Model Soup в {args.output_soup}...")
    peft_model.save_pretrained(args.output_soup)
    logger.info("✅ Model Soup успешно создан!")
