...
class SolverConfig:
    ...
    # Rendering mode:
    # - "training": concise successful reasoning path for SFT/RL traces.
    # - "debug": full replay renderer kept for diagnosing solver behavior.
    render_mode: Any = "training"

    # Search policy
    use_fixed_concat_rules: Any = True
    direct_solve_fixed_concat_target: Any = True
    ...
    # Training renderer defaults: do not teach rejected DFS branches.
    include_search_branches: Any = False
    include_failed_attempt_events: Any = False
    include_no_change_events: Any = False
    include_rule_reject_summary: Any = False
    include_ambiguity_audit: Any = False
    show_current_domains: Any = False
    show_projection_derivation: Any = False
    show_supported_projection: Any = False
    show_support_counts: Any = False
    show_projection_reason: Any = False
    show_projection_search_space: Any = False
    causal_reject_blocks: Any = False
    render_successful_branch_assignments: Any = True
    max_training_domain_steps: Any = 10000000

    # Debug/full renderer knobs retained for local analysis.
...
class CryptarithmSolver:
    ...
    def solve(self, examples_text: Any, target_text: Optional[Any] = None, timeout_seconds: Optional[float] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            fixed_combo: Dict[str, Rule] = {}
            if self._cfg("use_fixed_concat_rules", True):
                fixed_combo = self._fixed_concat_rules(by_op, candidates)

            domains: Domains = {s: set(range(10)) for s in symbols}
            ordered_ops = sorted(
                (op for op in candidates if op not in fixed_combo),
                key=lambda op: (0 if op == target_op else 1, len(candidates[op]), -len(by_op[op]), op),
            )
            ...
            trace.add_event(
                ReplayEvent(
                    ...
                    decision="order",
                    meta={
                        "ordered_ops": tuple(ordered_ops),
                        "target_op": target_op,
                        "fixed_rules": tuple(sorted((op, self._rule_name(rule)) for op, rule in fixed_combo.items())),
                    },
                )
            )

            # If the target operator is a transparent positional concat, no digit
            # values are needed to answer the target.  Do not spend the training
            # trace assigning arbitrary injective digits just to verify unrelated
            # examples; that teaches noise instead of mapping reasoning.
            target_fixed_rule = fixed_combo.get(target_op)
            if (
                self._cfg("direct_solve_fixed_concat_target", True)
                and target_fixed_rule is not None
                and target_fixed_rule.name == "cat"
            ):
                answer = self._concat_visible_output(target_left, target_right, target_fixed_rule)
                events = trace.finish()
                self.stats.replay_events = len(events)
                solution_lines = self._render_fixed_concat_target_solution(
                    equations=equations,
                ...
                return {
                    ...
                    "mapping": {},
                    "rules": {op: self._rule_name(rule) for op, rule in fixed_combo.items()},
                    "fixed_concat_rules": {op: self._rule_name(rule) for op, rule in fixed_combo.items()},
                    "render_mode": self._cfg("render_mode", "training"),
                    "audit": {"skipped": True, "reason": "fixed transparent concat target needs no digit map"},
                    "stats": self.stats.__dict__,
                    "elapsed_seconds": round(time.time() - started, 4),
                    "training_category": "cryptarithm.fixed_concat_target_direct",
                }

            assignment, combo = self._search_rules(
                ordered_ops,
                candidates,
                equations,
                domains,
                ...
                trace,
                0,
                target_context=(target_left, target_op, target_right),
            )
    ...
    def _concat_visible_output(self, left: str, right: str, rule: Rule) -> str:
        """Visible output for positional concat without solving any digit map."""
        return (right + left) if rule.reverse else (left + right)

    def _render_fixed_concat_target_solution(
        self,
        ...
        """Training renderer for target operators that are transparent concat.

        For these items the target answer is determined by visible symbol order,
        so assigning arbitrary digits is actively harmful for SFT: it teaches fake
        mapping work.  The trace therefore proves only the concat orientation and
        applies it directly to the target.
        """
        lines: List[str] = []
        add = lines.append
        by_op: Dict[str, List[Equation]] = {}
        ...
        target_rule = fixed_combo[target_op]
        add("Cryptarithm training trace")
        add("The target operator is a transparent positional concat, so the target can be solved without assigning digit values.")
        add("Digit mapping search is skipped for this item: concat copies visible symbols and does not constrain numeric digit values.")
        add("")
        ...
        add("Symbols")
        add("Digit symbols present: " + ", ".join(repr(ch) for ch in symbols))
        add("No digit map is selected because it is not needed for a fixed concat target.")
        add("")

        add("Transparent concat")
        for op in sorted(fixed_combo):
            rule = fixed_combo[op]
            add(f"Operator {repr(op)} is fixed as {self._rule_name(rule)} by visible positional equality.")
            for eq in by_op.get(op, []):
                expected = self._cat_expected_output(eq, rule)
                add(f"  {eq.display()}: expected positional output {repr(expected)}, observed {repr(eq.result)} -> MATCH")
        add("")

        add("Structural family filtering")
        for op in sorted(by_op):
            add(f"Operator {repr(op)}")
            add("  examples: " + "; ".join(eq.display() for eq in by_op[op]))
            for family, names in self._rule_family_order():
                fam_rules = [r for r in candidates.get(op, []) if r.name in names]
                if not fam_rules:
                    add(f"  Family {family}: no structurally valid candidate remains")
                else:
                    add("  Family " + family + ": structurally valid candidates = " + ", ".join(self._rule_name(r) for r in fam_rules))
            if op in fixed_combo:
                add(f"  selected rule: {self._rule_name(fixed_combo[op])} (fixed transparent concat)")
            else:
                add("  not solved: this operator is irrelevant to the fixed-concat target answer")
        add("")

        add("Target")
        add(f"Target operator {repr(target_op)} uses fixed rule {self._rule_name(target_rule)}.")
        if target_rule.reverse:
            add(f"rev/cat writes the right operand followed by the left operand: {target_right}+{target_left} -> {answer}")
        else:
            add(f"std/cat writes the left operand followed by the right operand: {target_left}+{target_right} -> {answer}")
    ...
    def _render_training_solution(
        ...
        """Training CoT renderer.

        This is intentionally closer to ASTBruteForceSolver's training trace than
        to the old CSP debug replay:
          header -> search space -> family/rule search -> domain projections ->
          selected map -> verification -> target.

        The solver may still use DFS internally, but the training text does not
        teach rejected DFS branches.  It teaches the reusable procedure:
        column constraints -> supported local assignments -> domain narrowing ->
        AllDifferent -> singleton digit map.
        """
        lines: List[str] = []
        add = lines.append
        by_op: Dict[str, List[Equation]] = {}
        ...
        fixed_ops = {op for op in combo if self._is_fixed_concat_op(op, combo, ordered_ops)}

        add("Cryptarithm training trace")
        add("We need to infer the digit map and the operator rules needed for the target.")
        add("This trace uses family-first digit-domain reasoning: each family block looks for supported digit assignments and narrows domains.")
        add("Rejected DFS branches are not part of the training trace; the proof below follows the successful constraint path.")
        add("")
        ...
        for i, eq in enumerate(equations, 1):
            signed = "signed" if eq.has_sign else "unsigned"
            add(f"{i}. left={eq.left}, op={repr(eq.op)}, right={eq.right}, output={eq.result}, output_format={signed}")
        add("")

        add("Symbols")
        add("Digit symbols: " + ", ".join(repr(ch) for ch in symbols))
        add("Operator symbols: " + ", ".join(repr(op) for op in sorted(by_op)))
        add("AllDifferent: every digit symbol must take a different decimal digit.")
        add("No-leading-zero: multi-digit decoded numbers may not start with 0.")
        add("")

        add("Transparent concat")
        add("Concat is handled before arithmetic search because positional concat copies symbols and normally gives no digit-value information.")
        if fixed_ops:
            for op in sorted(fixed_ops):
                rule = combo[op]
                add(f"Operator {repr(op)} is fixed as {self._rule_name(rule)} and skipped during digit-domain narrowing.")
                for eq in by_op.get(op, []):
                    expected = self._cat_expected_output(eq, rule)
                    add(f"  {eq.display()}: expected positional output {repr(expected)}, observed {repr(eq.result)} -> MATCH")
        else:
            add("No operator has a uniquely forced positional concat rule.")
        add("")

        add("Search space")
        add("Search policy: family-first digit-domain search, not candidate-level DFS in the training text.")
        add("A family block groups related rules and checks whether any local digit assignment can satisfy the examples under current domains.")
        add("A projection keeps exactly the digits that occur in at least one supported local assignment for that equation/rule.")
        add("After each projection, AllDifferent removes fixed digits from other symbol domains.")
        add("Operator priority: target operator and more constrained operators first; transparent concat is removed before this order.")
        add("Rule-combo search order: " + (", ".join(repr(op) for op in ordered_ops) if ordered_ops else "none; all operators were fixed before search"))
        add("Family order:")
        for i, (family, names) in enumerate(self._rule_family_order(), 1):
            add(f"{i}. {family}: " + ", ".join(names))
        add("Read directions: std reads symbols left-to-right; rev reads them right-to-left and encodes the result right-to-left.")
        add("")
        ...
        for op in sorted(by_op):
            ...
            selected = combo[op]
            add(f"Operator {repr(op)}")
            add("  examples: " + "; ".join(eq.display() for eq in eqs))
            for family, names in self._rule_family_order():
                fam_rules = [r for r in candidates.get(op, []) if r.name in names]
                if not fam_rules:
                    add(f"  Family {family}: no structurally valid candidate remains")
                    ...
                    add("  Family " + family + ": structurally valid candidates = " + ", ".join(self._rule_name(r) for r in fam_rules))
            if op in fixed_ops:
                add(f"  selected rule: {self._rule_name(selected)} (fixed transparent concat)")
            else:
                add(f"  selected rule in successful proof: {self._rule_name(selected)}")
        add("")

        search_lines = self._render_training_rule_and_domain_search(events, combo)
        if search_lines:
            add("Rule and digit-domain search")
            lines.extend(search_lines)
            add("")

        add("Selected rules")
        for op in sorted(combo):
            suffix = " (transparent concat fixed before search)" if op in fixed_ops else ""
            add(f"{repr(op)} -> {self._rule_name(combo[op])}{suffix}")
        add("")
        ...
        add(f"Target operator {repr(target_op)} uses selected rule {self._rule_name(target_rule)}.")
    ...
    def _render_training_rule_and_domain_search(self, events: Sequence[ReplayEvent], combo: Dict[str, Rule]) -> List[str]:
        """Render the successful constraint path as reusable mapping reasoning."""
        lines: List[str] = []
        block_no = 0
        current_rule: Optional[str] = None
        current_op: Optional[str] = None
        rejected_rules_by_key: Dict[Tuple[Any, Any], List[Tuple[str, str]]] = {}
        for ev in events:
            if ev.kind == "RULE_REJECT":
                meta = ev.meta or {}
                key = (meta.get("op"), meta.get("depth"))
                rejected_rules_by_key.setdefault(key, []).append((str(meta.get("rule", "unknown-rule")), str(ev.reason or "failed downstream")))

        for event in events:
            kind = event.kind
            if event.decision == "reject" or kind.endswith("_REJECT") or kind in {"MAP_REJECT", "RULE_REJECT", "COMBO_REJECT", "DIGIT_REJECT", "TUPLE_REJECT"}:
                continue
            if kind in {"RULE_ORDER", "MODULAR_SKIP", "COMBO_ACCEPT"}:
                continue

            if kind == "RULE_TRY":
                meta = event.meta or {}
                current_op = meta.get("op")
                current_rule = meta.get("rule")
                if current_op is None or current_rule is None:
                    continue
                family = self._family_name_for_rule_name(str(current_rule).split("/", 1)[-1])
                block_no += 1
                lines.append(f"Family/domain block {block_no}: operator {repr(current_op)} tests candidate families")
                lines.append(f"  selected successful candidate: {current_rule}")
                lines.append(f"  selected family: {family}")
                support_table = meta.get("support_table")
                if support_table:
                    lines.append("  family support table under current domains:")
                    lines.extend(self._render_training_support_table(support_table, current_rule, indent="    "))
                rejected_before = rejected_rules_by_key.get((current_op, meta.get("depth")), [])
                lines.extend(self._render_training_rule_selection_explanation(support_table, current_rule, rejected_before, indent="  "))
                continue

            if kind == "COMBO_PROPAGATE":
                combo_items = event.meta.get("combo", ()) if event.meta else ()
                combo_text = "; ".join(f"{repr(op)}->{rule}" for op, rule in combo_items)
                block_no += 1
                lines.append(f"Family/domain block {block_no}: all required operator rules are assigned")
                lines.append(f"  combo: {combo_text}")
                lines.append("  continue domain propagation until every digit domain is singleton")
                continue
            ...
            if kind in {"NO_LEADING_ZERO", "ALLDIFFERENT"}:
                if not event.changes:
                    continue
                block_no += 1
                label = "No-leading-zero" if kind == "NO_LEADING_ZERO" else "AllDifferent"
                lines.append(f"Family/domain block {block_no}: {label}")
                if event.reason:
                    lines.append(f"  reason: {event.reason}")
                lines.append(f"  update: {self._changes_text(event.changes)}")
                continue

            if kind in {"COLUMN_PROJECT", "FULL_PROJECT", "PROJECTION"}:
                if event.decision not in {"keep", "select"} or not event.changes:
                    continue
                block_no += 1
                lines.extend(self._render_training_projection_block(event, block_no, current_op, current_rule))
                continue

            if kind == "BRANCH_SELECT":
                if not self._cfg("render_successful_branch_assignments", True):
                    continue
                block_no += 1
                lines.append(f"Family/domain block {block_no}: accepted digit decision point")
                lines.append("  unresolved domains remain after propagation")
                if event.reason:
                    lines.append(f"  policy: {event.reason}")
                meta = event.meta or {}
                symbol = meta.get("symbol")
                domain = meta.get("domain")
                if symbol is not None and domain is not None:
                    lines.append(f"  selected symbol domain: D[{repr(symbol)}]={self._dom(domain)}")
                lines.append("  rejected values are omitted; the next branch commit is the successful path")
                continue
            ...
            if kind == "TUPLE_BRANCH_SELECT":
                if not self._cfg("render_successful_branch_assignments", True):
                    continue
                block_no += 1
                lines.append(f"Family/domain block {block_no}: accepted column-tuple decision point")
                if event.label:
                    lines.append(f"  label: {event.label}")
                if event.constraint:
                    lines.append(f"  formula: {event.constraint}")
                if event.reason:
                    lines.append(f"  policy: {event.reason}")
                meta = event.meta or {}
                tuple_count = meta.get("tuple_count")
                product_space = meta.get("product_space")
                if tuple_count is not None and product_space is not None:
                    lines.append(f"  tuple search space: {tuple_count} supported tuples instead of raw domain product {product_space}")
                tuples = meta.get("tuples", ())
                if tuples:
                    rendered = []
                    for tup in tuples:
                        rendered.append("{" + ", ".join(f"{repr(ch)}={d}" for ch, d in tup) + "}")
                    lines.append("  candidate tuples: " + "; ".join(rendered))
                lines.append("  rejected tuples are omitted; the next tuple commit is the successful path")
                continue

            if kind in {"TUPLE_TRY", "DIGIT_TRY"}:
                if not self._cfg("render_successful_branch_assignments", True):
                    continue
                if not event.changes:
                    continue
                block_no += 1
                if kind == "TUPLE_TRY":
                    lines.append(f"Family/domain block {block_no}: accepted tuple assignment")
                else:
                    lines.append(f"Family/domain block {block_no}: accepted digit assignment")
                if event.reason:
                    lines.append(f"  accepted assignment: {event.reason}")
                lines.append("  apply update: " + self._changes_text(event.changes))
                lines.append("  decision: propagate this successful assignment")
                continue
            ...
            if kind == "MAP_ACCEPT":
                ...
                if assignment:
                    block_no += 1
                    lines.append(f"Family/domain block {block_no}: singleton domains verify the examples")
                    lines.append("  map: " + "; ".join(f"{repr(ch)}={d}" for ch, d in assignment))
                continue

        return lines

    def _render_training_support_table(self, support_table: Any, selected_rule: str, indent: str = "    ") -> List[str]:
        lines: List[str] = []
        for group in support_table or ():
            rows = list(group or ())
            if not rows:
                continue
            family = self._family_name_for_rule_name(str(rows[0][0]).split("/", 1)[-1]) if rows and rows[0] else "unknown_family"
            lines.append(f"{indent}Family {family}:")
            for row in rows:
                try:
                    rule_name, supported, counts, note = row
                except Exception:
                    continue
                mark = " <- selected full-continuation rule" if str(rule_name) == str(selected_rule) else ""
                if counts:
                    count_text = ", ".join(str(c) for c in counts)
                    count_text = f"per-example support counts [{count_text}]"
                else:
                    count_text = "no support counts"
                status = "supported" if supported == "yes" else "no support" if supported == "no" else str(supported)
                note_text = f"; {note}" if note else ""
                lines.append(f"{indent}  {rule_name}: {status}; {count_text}{note_text}{mark}")
        return lines

    def _support_table_supported_rule_names(self, support_table: Any) -> List[str]:
        names: List[str] = []
        for group in support_table or ():
            for row in group or ():
                try:
                    rule_name, supported, _counts, _note = row
                except Exception:
                    continue
                if supported == "yes":
    ...
    def _render_training_rule_selection_explanation(
        self,
        support_table: Any,
        selected_rule: Any,
        rejected_before: Sequence[Tuple[str, str]],
        indent: str = "  ",
    ) -> List[str]:
        """Explain selected rule beyond local support counts.

        The support table is deliberately local: it asks whether each candidate can
        satisfy each example under current domains.  Several rules can pass that
        local test.  The selected rule is the first candidate in deterministic
        rule order whose downstream propagation through the remaining operators
        and digit search reaches a verifying singleton map.  This keeps the trace
        honest without printing full rejected DFS branches.
        """
        lines: List[str] = []
        selected = str(selected_rule)
        supported = self._support_table_supported_rule_names(support_table)
        other_supported = [name for name in supported if name != selected]
        lines.append(f"{indent}rule-selection explanation:")
        lines.append(f"{indent}  local support is necessary but not sufficient: support counts are per-example/domain checks, not proof of one global digit map.")
        if rejected_before:
            shown = []
            for rule_name, reason in rejected_before[:8]:
                # Keep this short: the detailed failed branch is intentionally not part of training CoT.
                compact_reason = reason.split(": ", 1)[-1]
                shown.append(f"{rule_name} failed downstream ({compact_reason})")
            suffix = "" if len(rejected_before) <= 8 else f"; plus {len(rejected_before)-8} earlier failures"
            lines.append(f"{indent}  candidates tried before {selected}: " + "; ".join(shown) + suffix)
        else:
            lines.append(f"{indent}  no earlier candidate in this operator's order reached this point before {selected}.")
        if other_supported:
            lines.append(f"{indent}  other locally supported candidates: " + ", ".join(other_supported))
            lines.append(f"{indent}  they are not selected here because the training trace follows the first full downstream continuation that verifies all examples.")
        else:
            lines.append(f"{indent}  no other candidate in the table has local support under the current domains.")
        lines.append(f"{indent}  decision: select {selected} as the successful full-continuation rule; rejected continuations remain omitted.")
        return lines

    def _render_training_projection_block(
        self,
        event: ReplayEvent,
        block_no: int,
        ...
        lines: List[str] = []
        meta = event.meta or {}
        deriv = meta.get("projection_derivation", {}) if isinstance(meta.get("projection_derivation", {}), dict) else {}
        rule_name = self._rule_name_from_projection_label(event.label) or current_rule or "unknown-rule"
        family = self._family_name_for_rule_name(str(rule_name).split("/", 1)[-1])
        label = event.label or "local arithmetic projection"
        support_count = event.count
        if support_count is None and deriv:
            support_count = deriv.get("satisfying_count")

        lines.append(f"Family/domain block {block_no}: projection by {rule_name}")
        ...
        if support_count is not None:
            lines.append(f"  supported local assignments: {support_count}")

        deriv_lines = list(deriv.get("derivation_lines", ()) or ())
        if deriv_lines:
            lines.append("  column reasoning:")
            for detail in deriv_lines:
                lines.append(f"    {detail}")

        if event.supported is not None:
            lines.append("  supported digit projection: " + self._domain_text_map(event.supported))
        if event.changes:
            lines.append("  apply projection: " + self._changes_text(event.changes))
        return lines

    @staticmethod
    def _rule_name_from_projection_label(label: str) -> Optional[str]:
        if not label:
            return None
        marker = " via "
    ...
    def _family_name_for_rule_name(self, rule_name: str) -> str:
        base = str(rule_name).split("/", 1)[-1]
        for family, names in self._rule_family_order():
            if base in names:
                return family
    ...
    def _render_replay_solution(
        ...
        if self._cfg("render_mode", "training") == "training":
            return self._render_training_solution(
                equations=equations,
                target_expr=target_expr,
                target_left=target_left,
                target_op=target_op,
    ...
    def _fixed_concat_rules(self, by_op: Dict[str, List[Equation]], candidates: Dict[str, List[Rule]]) -> Dict[str, Rule]:
        """Promote obvious concatenation operators out of rule-combo search.

        Concatenation does not reveal numeric digit values.  If every example for an
        operator matches exactly one positional concat orientation, the operator is
        transparent: it can be fixed safely and removed from the expensive rule DFS.

        Ambiguous cases are deliberately left in the normal search.  For example,
        if both std/cat and rev/cat could fit, the target orientation is not fixed.
        """
        fixed: Dict[str, Rule] = {}
        for op, eqs in by_op.items():
            cat_rules = [r for r in candidates.get(op, ()) if r.name == "cat"]
            if not cat_rules:
                continue
            forced = [
                rule for rule in cat_rules
                if all((not eq.has_sign) and self._cat_expected_output(eq, rule) == eq.result for eq in eqs)
            ]
            if len(forced) == 1:
                fixed[op] = forced[0]
        return fixed

    def _is_fixed_concat_op(self, op: str, combo: Dict[str, Rule], ordered_ops: Sequence[str]) -> bool:
        rule = combo.get(op)
        return rule is not None and rule.name == "cat" and op not in set(ordered_ops)
    ...
    def _propagate(self, equations: Sequence[Equation], combo: Dict[str, Rule], domains: Domains, trace: ReplayTrace, target_context: Optional[Tuple[str, str, str]] = None) -> bool:
        while True:
    ...
    def _rule_family_order(self) -> List[Tuple[str, List[str]]]:
        # Training/search priority follows the useful arithmetic signal first.
        # Concat is intentionally last because deterministic concat is fixed
        # before DFS and non-fixed concat gives no digit-value information.
        return [
    ...
    def _local_support_count_for_rule(self, eq: Equation, rule: Rule, domains: Domains) -> Tuple[Optional[int], str]:
        """Return a local support count for one equation/rule under current domains.

        The count is local to this equation. It is used only for the training
        renderer's family support table, not as a search decision by itself.
        """
        if rule.name == "cat":
            ok = (not eq.has_sign) and self._cat_expected_output(eq, rule) == eq.result
            return (1 if ok else 0), "positional"

        deriv = self._derive_rule_specific_support(eq, rule, domains)
        if deriv is not None:
            return int(deriv.get("count", 0)), str(deriv.get("method", "rule-specific DP"))

        scope = list(dict.fromkeys(eq.left + eq.right + eq.result))
        if len(scope) > int(self._cfg("max_rule_specific_variables", 8)):
            return None, "too many local variables for cheap support table"
        ok, _supported, count, _witnesses, _space = self._supports_with_witnesses(
            scope,
            domains,
            lambda local: self._equation_matches(eq, rule, local),
            max_witnesses=0,
        )
        return int(count if ok else 0), "generic local enumeration"

    def _operator_support_table_for_training(
        self,
        op: str,
        candidate_rules: Sequence[Rule],
        equations: Sequence[Equation],
        domains: Domains,
        combo_prefix: Dict[str, Rule],
        target_context: Optional[Tuple[str, str, str]],
    ) -> Tuple[Tuple[Tuple[str, str, Tuple[Any, ...], str], ...], ...]:
        """Build grouped local support rows for the successful RULE_TRY event.

        Rows are grouped by family and include all structurally kept candidates for
        the current operator.  Counts are local per example, after applying cheap
        no-leading-zero / AllDifferent constraints for the prefix plus this rule.
        This gives the model an explicit rule-selection table without showing
        rejected DFS branches.
        """
        op_examples = [eq for eq in equations if eq.op == op]
        grouped: Dict[str, List[Tuple[str, str, Tuple[Any, ...], str]]] = {}

        for rule in candidate_rules:
            family = self._family_name_for_rule_name(rule.name)
            rule_name = self._rule_name(rule)
            local_domains = self._copy_domains(domains)
            local_combo = dict(combo_prefix)
            local_combo[op] = rule
            silent = SilentReplayTrace()
            note_parts: List[str] = []
            try:
                if not self._leading_zero(equations, local_combo, local_domains, silent):
                    row = (rule_name, "no", tuple(), "leading-zero contradiction")
                    grouped.setdefault(family, []).append(row)
                    continue
                if target_context is not None:
                    tl, top, tr = target_context
                    if not self._target_leading_zero(tl, top, tr, local_combo, local_domains, silent):
                        row = (rule_name, "no", tuple(), "target leading-zero contradiction")
                        grouped.setdefault(family, []).append(row)
                        continue
                if not self._alldifferent(local_domains, silent):
                    row = (rule_name, "no", tuple(), "AllDifferent contradiction before support count")
                    grouped.setdefault(family, []).append(row)
                    continue
                ...
                for eq in op_examples:
                    count, method = self._local_support_count_for_rule(eq, rule, local_domains)
                    methods.add(method)
                    if count is None:
                        ...
                        if count <= 0:
                            supported = False
                note = ", ".join(sorted(m for m in methods if m))
                row = (rule_name, "yes" if supported else "no", tuple(counts), note)
                grouped.setdefault(family, []).append(row)
            except TimeoutError:
                raise
            except Exception as exc:
                row = (rule_name, "unknown", tuple(), f"support-table error: {type(exc).__name__}")
                grouped.setdefault(family, []).append(row)

        ordered: List[Tuple[Tuple[str, str, Tuple[Any, ...], str], ...]] = []
        for family, names in self._rule_family_order():
            rows = grouped.get(family, [])
            if rows:
                ordered.append(tuple(rows))
        for family in sorted(set(grouped) - {fam for fam, _ in self._rule_family_order()}):
            ordered.append(tuple(grouped[family]))
    ...
    def _search_rules(
        ...
        for rule in candidates[op]:
            ...
            domains2 = self._copy_domains(domains)
            trace2 = ReplayTrace(self.config)
            support_table = self._operator_support_table_for_training(
                op=op,
                candidate_rules=candidates[op],
                equations=equations,
                domains=domains,
                combo_prefix=combo,
